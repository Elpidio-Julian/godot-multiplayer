...
public partial class NetworkCharacter : CharacterBody3D, INetworkCharacter
{
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		if (this.PreviousCrouchLevel != this.NetworkCrouchingLevel && this.Shape?.Shape != null)
		{
			...
			transform.Origin = new Vector3(transform.Origin.X, this.OriginalYPosition + yPos, transform.Origin.Z);
			transform.Origin = new Vector3(transform.Origin.X, Mathf.Clamp(transform.Origin.Y, (this.ShapeHeight / 2) * -1f, this.OriginalYPosition), transform.Origin.Z);
			...
			else
			{
				...
				if (!default(PlayerState).Equals(state))
				{
					...
					this.DebugMesh.GlobalTransform = new Transform3D(
						new Basis(state.GetVar<Quaternion>(this, "NetworkRotation")),
	...
	public virtual RayCastHit DetechtHit(float range)
	{
		...
		var basis = Basis.FromEuler(headViewRotation);
		...
		var attackTransformFrom = new Transform3D(new Basis(command.GetVar<Quaternion>(this, "NetworkRotation")), attackPosition);
